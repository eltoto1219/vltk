...
from mllib.models import (get_b16_config, get_b32_config, get_h14_config,
...
class MlpBlock(nn.Module):
    ...
    def apply(
        ...
        x = nn.Dense(
            inputs, mlp_dim, dtype=dtype, kernel_init=kernel_init, bias_init=bias_init
        )
        x = nn.gelu(x)
        x = nn.dropout(x, rate=dropout_rate, deterministic=deterministic)
        output = nn.Dense(
            x, actual_out_dim, dtype=dtype, kernel_init=kernel_init, bias_init=bias_init
        )
...
KNOWN_MODELS = {
    name: VisionTransformer.partial(**config) for name, config in CONFIGS.items()
}
