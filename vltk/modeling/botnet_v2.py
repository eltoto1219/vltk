...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride=1, mhsa=False):
        print(f"num_blocks {num_blocks}, planes: {planes}, mhsa {mhsa}")
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        print("layer 3 out -------------------------", out.shape)
        out = self.layer4(out)
        print("layer 4 out -----------------------------", out.shape)
...
def ResNet50(num_classes=1000, resolution=(256, 256)):
    return ResNet(
        Bottleneck, [3, 4, 6, 3], num_classes=num_classes, resolution=resolution
    )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, mhsa=False, resolution=None):
        ...
        if not mhsa:
            ...
            self.conv2.append(
                MHSA(planes, width=int(resolution[0]), height=int(resolution[1]))
            )
            if stride == 2:
    ...
    def forward(self, x):
        print("start block shape", x.shape)
        out = F.relu(self.bn1(self.conv1(x)))
        print("first conv of block", out.shape)
        out = F.relu(self.bn2(self.conv2(out)))
        print("second conv of bock", out.shape)
        out = self.bn3(self.conv3(out))
        print("third conv of bock", out.shape)
        out += self.shortcut(x)
        out = F.relu(out)
        return out
