...
class BatchInfo:
    def __init__(self, config):
        ...
        self.visn_keys = set()
        self.lang_keys = set()
        self.max_shapes_per_item = defaultdict(tuple)
        self.all_keys = set()

    def update_visn_lang_keys(self, lang_entry=None, visn_entry=None):
        if lang_entry is not None:
            self.lang_keys = self.lang_keys.union(set(lang_entry.keys()))
        if visn_entry is not None:
    ...
    def update_entry_keys(self, entry):
        ...
        for k, v in entry.items():
            ...
            if isinstance(v, torch.Tensor):
                current_shape = tuple(v.shape)
                before_shape = self.max_shapes_per_item[k]
                if not before_shape or (
                    before_shape and sum(before_shape) < sum(current_shape)
                ):
                    self.max_shapes_per_item[k] = current_shape

        self.uniq_keys = self.uniq_keys.union(entry_keys)
        self.all_keys = self.all_keys.union(self.uniq_keys)
    ...
    def clear(self):
        ...
        self.max_shapes_per_item.clear()
        ...
        rep = json.dumps(
            {
                ...
                "max_shapes_per_item": dict(self.max_shapes_per_item),
            },
            indent=4,
        )
        return rep
...
def collate(
    ...
    if not batch_info.sparse_keys:
        batch = collate_homogeneous(columns, config, batch_info, pad_id, sep_id)
    else:
        ...
        if k not in (vltk.imgid, vltk.qid) and not isinstance(columns[0][k], str):
            batch[k] = torch.stack(
                [
                    utils.pad_tensor(
                        i.get(k),
                        pad_shape=batch_info.max_shapes_per_item[k],
                        pad_value=pad_id,
                        debug=k,
                    )
                    for i in columns
    ...
    def __init__(self, config, is_train=False, **kwargs):
        ...
        super().__init__(
            ...
            collate_fn=lambda x: collate(
                ...
                sep_id=dataset.tokenizer.sep_id,
                config=config,
                batch_info=batch_info,
            ),
            drop_last=drop_last,
